/*
 *14331083 ，HanYifan ，Version2.0 ，11/28 22:45;
 *
 *适用于网页中任意数目各规格的表格(暂时只测过一两个，orz...)；
 *网页原来的排序会被自动覆盖；
 *方法控制在10行左右；
 *
 ...
 *，             时间:2015/11/28/12:00
 *
 *（将ReadMe文件代码复制到chrome的console后按回车即可,ReadMe文件代码与该js文件差别在于无需window.onload操作而是直接执行）
 *
 *有错漏欢迎指正；
 */
...
function SortContent() {
	for (var i = 0, j = 0; i < Global.td_length[Global.s]; i++) {
		if (i % Global.th_length[Global.s] == Global.ind) {
			...
			if (!isNaN(Number(Global.contents[j]))) Global.contents[j] = Number(Global.contents[j]);
			j++;
		}          
	}
	if (Global.up[Global.s][Global.ind]) {
...
function Update() {
	...
	for (var i =0; i < Global.row; i++) {
		for (var j = Global.ind; j < Global.td_length[Global.s]; j +=Global.th_length[Global.s]) {
			...
			if (!isNaN(Global.contents[i])) compare = Number(compare);
